 ...
 * */
function menuEditor(idSelector, settings, optionId) {
 var $main = $("#" + idSelector);
 ...
 $('#btnOut').on('click', function() {
  ...
  $.ajax({
   method: 'PUT',
   url: `/admin/options/${optionId}`,
   dataType: 'json',
   data: {
    'commit': 'Update Menu',
    'admin_option': { 'value': str }
   },
 ...
 function updateItem() {
  ...
  itemEdit.data('li_class', $("#mnu_li_class").val());
  itemEdit.data('ahref_class', $("#mnu_ahref_class").val());
  itemEdit.data('options', $("#mnu_options").val());
  reset();
 }
 ...
 function addItem() {
  ...
  var text = $("#mnu_text").val();
  var btnEdit = TButton({
   classCss: 'btn btn-default btn-xs btnEdit',
   text: labelEdit
  });
  var btnRemv = TButton({
 ...
 function createMenu(arrayItem, depth) {
  ...
  $.each(arrayItem, function(k, v) {
   ...
   $li.addClass('list-group-item').data('text', v.text).data('icon', v.icon)
      .data('target', v.target).data('href', v.href).data('li_class', v.li_class)
      .data('options', v.options).data('ahref_class', v.ahref_class);
   var $div = $('<div>');
